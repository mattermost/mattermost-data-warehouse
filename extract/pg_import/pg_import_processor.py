...
class PgImporter(object):
    def __init__(self, source_table, destination_table, post_process_file):
        ...
        self.sf_engine = snowflake_engine_factory(
            os.environ, "TRANSFORMER", self.source_schema
        )
    ...
    def _create_pg_table(self, conn):
        ...
        for column in column_specs:
            columns.append(
                f"{column[0].lower()} {SnowflakeToPgDataTypeMapper.get_pg_type(column[1].lower(), column[2], column[3])}"
            )
        ...
        # We should recreate this table every time we run the job so it has the latest columns
        conn.execute(f"drop table {self.destination_schema}.{self.destination_table};")
    ...
    def _unload_to_s3(self):
        sf_unload_stage = os.getenv(
            "SNOWFLAKE_UNLOAD_STAGE", "analytics.mattermost.pg_stage"
        )
        file_name = f"{self.source_table}.csv"

        with self.sf_engine.begin() as conn:
            conn.execute(
                f"COPY INTO @{sf_unload_stage}/{file_name} from {self.source_schema}.{self.source_table} overwrite=true single=true max_file_size=4900000000;"
            )
    ...
    def _process_on_pg(self, file_name):
        ...
        with TemporaryFile() as f:
            ...
            try:
                ...
                cursor.copy_expert(
                    f"COPY {full_table} FROM STDIN WITH CSV NULL 'NULL'", f
                )
...
class PgImportProcessor(object):
    ...
    def run(self):
        for record in self._get_tables_to_process():
            ...
            logging.info(
                f"Running PG import for id {id} with source table {source_table}"
            )
            rows_affected = PgImporter(
                source_table, destination_table, post_process_file
            ).run()
