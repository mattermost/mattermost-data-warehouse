...
(
    ...
    FROM (
        ...
        UNION ALL
        SELECT USERAGENT AS CONTEXT_USERAGENT
        FROM ANALYTICS.MATTERMOST.DOWNLOADS
        WHERE USERAGENT IS NOT NULL
            AND USERAGENT NOT IN (SELECT CONTEXT_USERAGENT FROM analytics.WEB.user_agent_registry GROUP BY 1)
        GROUP BY 1
        UNION ALL
    ...
    if len(df) == 0: # CHECKS TO SEE IF THERE ARE ANY NEW CONTEXT_USERAGENTS TO INSERT INTO THE TABLE
        ...
        agent_lists = [browser, browser_version, op_sys, os_version, device_type, device_brand, device_model]
        for item in agent_lists:
            df = df.join(item)

        engine = snowflake_engine_factory(os.environ, 'TRANSFORMER', 'WEB')                       )
        ...
        df[0:16384 if len(df) > 16384 else len(df)].to_sql('user_agent_registry', con=connection, index=False, schema='WEB', if_exists='append')
        ...
        if i <= math.ceil(len(df)/16384):
            ...
            y=16384*2 # The end row of the dataframe slice to be inserted. Will autoincrement if more than 2 inserts are required.
            ...
            for n in range(math.ceil(len(df)/16384)-1):
                df[x:y if y < len(df) else len(df)].to_sql('user_agent_registry', con=connection, index=False, schema='WEB', if_exists='append')
                x=y
                y+=16384
                i+=1
