...
with release_versions as (
    select
        short_version,
        lag(short_version, 1) over(partition by release_number asc) as previous_release_version,
        lag(short_version, -1) over(partition by release_number asc) as next_release_version
    from
