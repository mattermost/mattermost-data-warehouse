...
WITH account_w_arr AS (
    ...
    FROM {{ ref('stg_salesforce_opportunity_line_item') }} oli
    LEFT JOIN {{ ref('stg_salesforce_opportunity') }} o ON o.opportunity_id = oli.opportunity_id
    LEFT JOIN {{ ref('stg_salesforce_account') }} a ON a.account_id = o.account_id
    WHERE
        opportunity.iswon
    ...
    FROM {{ ref('stg_salesforce_account') }} a
    LEFT JOIN {{ ref('stg_salesforce_opportunity') }} o ON a.account_id = o.account_id AND o.status_wlo__c = 'Won'
    LEFT JOIN {{ ref('stg_salesforce_opportunity_line_item') }} oli ON o.opportunity_id = oli.opportunity_id
    LEFT JOIN {{ ref('stg_salesforce_product2') }} ON oli.product2id = product2.product2_id
