...
WITH daily_user_agent_events AS (
    SELECT
      ...
      , COALESCE(events.user_id, events.context_server, events.context_traits_server) AS instance_id
      , events.user_actual_id AS user_id
      ...
      , 'COALESCE(events.user_id, events.context_server, events.context_traits_server)'
      , 'events.user_actual_id'
    ...
    FROM {{ ref('user_events_telemetry') }}        events
         JOIN ref('user_agent_registry') user_agent
