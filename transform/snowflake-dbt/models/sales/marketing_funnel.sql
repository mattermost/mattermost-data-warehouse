...
pipeline_created as (
    select 
        ...
        round(sum(case when created_by_segment = 'Federal' then opportunity_snapshot.amount else null end),2) as federal_pipeline,
        round(sum(case when created_by_segment like 'Commercial%' then opportunity_snapshot.amount else null end),2) as commercial_ae_pipeline,
        round(sum(case when created_by_segment like 'Enterprise%' then opportunity_snapshot.amount else null end),2) as enterprise_ae_pipeline,
        round(sum(case when created_by_segment = 'Commercial BDR' then opportunity_snapshot.amount else null end),2) as commercial_bdr_pipeline,
        round(sum(case when created_by_segment = 'Enterprise BDR' then opportunity_snapshot.amount else null end),2) as enterprise_bdr_pipeline,
...
new_logo_and_arr_creation as (
    select 
        ...
        round(sum(case when market_segment = 'Federal' then amount else null end),2) as federal_arr,
        round(sum(case when market_segment = 'Commercial' then amount else null end),2) as commercial_arr,
        round(sum(case when market_segment = 'Enterprise' then amount else null end),2) as enterprise_arr,
        round(avg(case when market_segment = 'Federal' then amount else null end),2) as federal_avg_amount,
        round(avg(case when market_segment = 'Commercial' then amount else null end),2) as commercial_avg_amount,
        round(avg(case when market_segment = 'Enterprise' then amount else null end),2) as enterprise_avg_amount,
        round(avg(case when market_segment = 'Federal' then age else null end),2) as federal_avg_age,
        round(avg(case when market_segment = 'Commercial' then age else null end),2) as commercial_avg_age,
        round(avg(case when market_segment = 'Enterprise' then age else null end),2) as enterprise_avg_age
    from {{ ref('opportunity') }}
    join {{ ref('opportunity_ext') }} on opportunity.sfid = opportunity_ext.opportunity_sfid
