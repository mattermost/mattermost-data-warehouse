    ...
    {{ dbt_utils.group_by(n=8) }}
     ...
     licenses as (
         SELECT
           ...
           , ld.start_date
           , ld.expire_date
           , CASE WHEN lead(ld.start_date, 1) OVER (PARTITION BY ld.date, ld.server_id, ld.company,
                  CASE WHEN REGEXP_SUBSTR(company, '[^a-zA-Z]TRIAL$') in ('-TRIAL', 'TRIAL', ' TRIAL') THEN TRUE 
                  WHEN DATEDIFF(day, ld.start_date, ld.expire_date) <= 35 THEN TRUE
                  ELSE FALSE END ORDER BY ld.start_date, ld.users) <= ld.expire_date
                    AND lead(ld.start_date, 2) OVER (PARTITION BY ld.date, ld.server_id, ld.company,
                  CASE WHEN REGEXP_SUBSTR(company, '[^a-zA-Z]TRIAL$') in ('-TRIAL', 'TRIAL', ' TRIAL') THEN TRUE 
                  WHEN DATEDIFF(day, ld.start_date, ld.expire_date) <= 35 THEN TRUE
                  ELSE FALSE END ORDER BY ld.start_date, ld.users) <= ld.expire_date
                    AND lead(ld.start_date, 3) OVER (PARTITION BY ld.date, ld.server_id, ld.company,
                  CASE WHEN REGEXP_SUBSTR(company, '[^a-zA-Z]TRIAL$') in ('-TRIAL', 'TRIAL', ' TRIAL') THEN TRUE 
                  WHEN DATEDIFF(day, ld.start_date, ld.expire_date) <= 35 THEN TRUE
                  ELSE FALSE END ORDER BY ld.start_date, ld.users) <= ld.expire_date
                    THEN lead(ld.start_date, 3) OVER (PARTITION BY ld.date, ld.server_id, ld.company,
                  CASE WHEN REGEXP_SUBSTR(company, '[^a-zA-Z]TRIAL$') in ('-TRIAL', 'TRIAL', ' TRIAL') THEN TRUE 
                  WHEN DATEDIFF(day, ld.start_date, ld.expire_date) <= 35 THEN TRUE
                  ELSE FALSE END ORDER BY ld.start_date, ld.users) - INTERVAL '1 DAY'
                  WHEN lead(ld.start_date, 1) OVER (PARTITION BY ld.date, ld.server_id, ld.company,
                  CASE WHEN REGEXP_SUBSTR(company, '[^a-zA-Z]TRIAL$') in ('-TRIAL', 'TRIAL', ' TRIAL') THEN TRUE 
                  WHEN DATEDIFF(day, ld.start_date, ld.expire_date) <= 35 THEN TRUE
                  ELSE FALSE END ORDER BY ld.start_date, ld.users) <= ld.expire_date
                    AND lead(ld.start_date, 2) OVER (PARTITION BY ld.date, ld.server_id, ld.company,
                  ...
                  WHEN DATEDIFF(day, ld.start_date, ld.expire_date) <= 35 THEN TRUE
                  ELSE FALSE END ORDER BY ld.start_date, ld.users) <= ld.expire_date
                    THEN lead(ld.start_date, 2) OVER (PARTITION BY ld.date, ld.server_id, ld.company,
                  CASE WHEN REGEXP_SUBSTR(company, '[^a-zA-Z]TRIAL$') in ('-TRIAL', 'TRIAL', ' TRIAL') THEN TRUE 
                  WHEN DATEDIFF(day, ld.start_date, ld.expire_date) <= 35 THEN TRUE
                  ELSE FALSE END ORDER BY ld.start_date, ld.users) - INTERVAL '1 DAY'
                  WHEN lead(ld.start_date, 1) OVER (PARTITION BY ld.date, ld.server_id, ld.company,
                  ...
                  WHEN DATEDIFF(day, ld.start_date, ld.expire_date) <= 35 THEN TRUE
                  ELSE FALSE END ORDER BY ld.start_date, ld.users) <= ld.expire_date
                    THEN lead(ld.start_date, 1) OVER (PARTITION BY ld.date, ld.server_id, ld.company,
                  CASE WHEN REGEXP_SUBSTR(company, '[^a-zA-Z]TRIAL$') in ('-TRIAL', 'TRIAL', ' TRIAL') THEN TRUE 
                  WHEN DATEDIFF(day, ld.start_date, ld.expire_date) <= 35 THEN TRUE
                  ELSE FALSE END ORDER BY ld.start_date, ld.users) - INTERVAL '1 DAY'
             ELSE
