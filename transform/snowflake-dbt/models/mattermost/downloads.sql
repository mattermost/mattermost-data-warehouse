...
WITH outliers         AS (
    SELECT
      ...
      , uri
      , bytessent
      , abs(bytessent - avg(bytessent) OVER (PARTITION BY date_trunc('year', logdate::DATE), uri)) AS diff
      , avg(bytessent) OVER (PARTITION BY date_trunc('year', logdate::DATE), uri)                  AS avg
      , max(bytessent) OVER (PARTITION BY date_trunc('year', logdate::DATE), uri)                  AS max
      , stddev(bytessent) OVER (PARTITION BY date_trunc('year', logdate::DATE), uri) * 1           AS std
      , count(bytessent) OVER (PARTITION BY date_trunc('year', logdate::DATE), uri)                AS count
      , CASE
            WHEN abs(bytessent - avg(bytessent) OVER (PARTITION BY date_trunc('year', logdate::DATE), uri)) >
                 stddev(bytessent) OVER (PARTITION BY date_trunc('year', logdate::DATE), uri) * 1 THEN TRUE
