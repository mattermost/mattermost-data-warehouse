     ...
     user_nps    AS (
         ...
         FROM {{ source('util','dates') }} d
              ...
              LEFT JOIN (SELECT
                             month
                           , TRIM(user_id)                                                                                AS user_id
                           , MAX(CASE WHEN server_id IS NOT NULL THEN month ELSE NULL END)  
                              OVER (PARTITION BY user_id ORDER BY month ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS server_month
                           , MAX(CASE WHEN license_id IS NOT NULL THEN month ELSE NULL END)
                              OVER (PARTITION BY user_id ORDER BY month ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS license_month
                           , MIN(last_score_date)
                              OVER (PARTITION BY user_id ORDER BY month ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS first_nps_date
                           , MAX(last_score_date)
                              OVER (PARTITION BY user_id ORDER BY month ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS last_nps_date
