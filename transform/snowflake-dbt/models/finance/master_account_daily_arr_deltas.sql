...
WITH master_account_daily_arr_deltas AS (
    SELECT
        ...
        CASE
            WHEN master_account_util_dates.day = (SELECT min(day) FROM finance.master_account_daily_arr AS original WHERE new_day.master_account_sfid = original.accounmaster_account_sfidt_sfid ) 
                THEN 0 
            ELSE  coalesce(previous_day.total_arr_norm,0)
        END  AS previous_day_total_arr_norm,
        CASE
            WHEN master_account_util_dates.day = (SELECT min(day) FROM finance.master_account_daily_arr AS original WHERE new_day.master_account_sfid = original.master_account_sfid ) 
                THEN true 
            ELSE  false 
