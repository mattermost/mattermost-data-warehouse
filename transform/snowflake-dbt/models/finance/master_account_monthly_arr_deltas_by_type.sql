...
WITH master_account_monthly_arr_deltas_by_type AS (
    SELECT
        ...
        sum(CASE WHEN master_account_new_arr THEN total_arr_norm_delta ELSE 0 END) AS total_arr_norm_new,
        sum(CASE WHEN not master_account_new_arr AND total_arr_norm_delta > 0 THEN total_arr_norm_delta ELSE 0 END) AS total_arr_norm_expansion,
        ...
        sum(CASE WHEN not master_account_new_arr AND total_arr_delta > 0 THEN total_arr_delta ELSE 0 END) AS total_arr_expansion,
