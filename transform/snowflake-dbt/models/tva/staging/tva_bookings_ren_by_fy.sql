...
), actual_bookings_ren_by_fy AS (
    SELECT 
        util.fiscal_year(closedate) AS fy,
        sum(bookings) AS total_bookings
    FROM bookings_ren
    GROUP BY 1
), bookings_ren_by_fy AS (
    SELECT 
        util.fiscal_year(month) AS fy,
        SUM(target) as target
    FROM {{ source('targets', 'bookings_ren_by_mo') }}
    GROUP BY 1
), tva_bookings_ren_by_fy AS (
    SELECT
        'bookings_ren_by_fy' as target_slug,
        bookings_ren_by_fy.fy,
        bookings_ren_by_fy.target,
        actual_bookings_ren_by_fy.total_bookings as actual,
        round((actual_bookings_ren_by_fy.total_bookings/bookings_ren_by_fy.target),2) as tva
    FROM bookings_ren_by_fy
    LEFT JOIN actual_bookings_ren_by_fy ON bookings_ren_by_fy.fy = actual_bookings_ren_by_fy.fy
)
