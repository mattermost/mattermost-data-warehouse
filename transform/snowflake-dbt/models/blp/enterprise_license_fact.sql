...
WITH enterprise_license_fact AS (
    SELECT 
        ...
        MAX(CASE WHEN license_daily_details.date = CONVERT_TIMEZONE('America/Los_Angeles',current_timestamp)::date - INTERVAL '1 days' THEN (license_daily_details.license_registered_deactivated_users) ELSE NULL END)::int AS current_max_license_registered_users,
