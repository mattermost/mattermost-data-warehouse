...
WITH tva_bookings_new_and_exp_by_segment_by_qtr AS (
    SELECT
        REPLACE(target_slug,'_mo_','_qtr_') AS target_slug,
        util.fiscal_year(tva_bookings_new_and_exp_by_segment_by_mo.month)|| '-' || util.fiscal_quarter(tva_bookings_new_and_exp_by_segment_by_mo.month) AS qtr,
...
SELECT * FROM tva_bookings_new_and_exp_by_segment_by_qtr