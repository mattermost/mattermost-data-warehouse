...
WITH w_end_date AS (
  ...
  SELECT
      ...
      SUM(CASE WHEN opportunitylineitem.product_line_type__c = 'Expansion' AND is_prorated_expansion__c IS NULL THEN totalprice WHEN is_prorated_expansion__c IS NOT NULL THEN 365 * totalprice/(end_date__c::date - start_date__c::date + 1) ELSE 0 END) AS sum_expansion_w_proration_amount,
