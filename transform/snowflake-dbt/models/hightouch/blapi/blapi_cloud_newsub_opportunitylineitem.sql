...
WITH newsub_oli as (
...
), cloud_oli_to_sync AS (
    SELECT DISTINCT
        customers_with_cloud_subs.*,
        newsub_oli.opportunitylineitem_sfid,
        COALESCE(
            newsub_oli.opportunitylineitem_external_id,
    ...
    LEFT JOIN newsub_oli
        ON invoices_blapi.id = newsub_oli.invoice_id
    LEFT JOIN {{ ref('opportunity') }}
        ON newsub_oli.opportunity_external_id = opportunity.dwh_external_id__c
    WHERE customers_with_cloud_subs.hightouch_sync_eligible
    AND invoices_blapi.stripe_id != 'upcoming'
    AND invoices_blapi.total > 0
)
SELECT * FROM cloud_oli_to_sync