...
with orgm_account_telemetry as (
    ...
    where 
        (account.arr_current__c, account.seats_licensed__c, account.health_score__c,account.latest_telemetry_date__c::date, account.seats_active_latest__c, account.seats_active_mau__c, account.server_version__c, account.seats_registered__c)
        is distinct from
        (account_arr_and_seats.total_arr, account_arr_and_seats.seats, account_health_score.health_score_w_override, orgm_account_telemetry.last_telemetry_date, orgm_account_telemetry.dau, orgm_account_telemetry.mau, orgm_account_telemetry.server_version, orgm_account_telemetry.registered_users)
)
