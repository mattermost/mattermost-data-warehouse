UPDATE orgm.account
    SET     
        ...
        seats_registered__c = orgm_account_telemetry.registered_users
...
WHERE orgm_account_telemetry.account_sfid = account.sfid
    ...
    AND (orgm_account_telemetry.dau,orgm_account_telemetry.mau,orgm_account_telemetry.last_telemetry_date,orgm_account_telemetry.server_version,orgm_account_telemetry.registered_users) IS DISTINCT FROM 
        (account.seats_active_latest__c,account.seats_active_mau__c,account.latest_telemetry_date__c,account.server_version__c, account.seats_registered__c);